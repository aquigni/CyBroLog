...
def _is_safety_relevant(record: CyBroLogRecord) -> bool:
    ...
    needles = ["external-send", "secret-access", "privilege", "destructive", "shared-wiki-mutation", "service-identity-promotion", "p0", "may=approved", "approval", "authn", "π"]
...
def _validate_p0(record: CyBroLogRecord, errors: list[str]) -> None:
    ...
    hay_norm = hay.casefold()
    risky = any(n in hay_norm for n in ["external-send", "secret-access", "privilege", "destructive", "shared-wiki-mutation", "service-identity-promotion"])
    if not risky:
        return
