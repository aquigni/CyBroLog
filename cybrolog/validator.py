...
def run_benchmark_suite() -> dict[str, Any]:
    ...
    cases = [
        ...
        "ψ=CL2.v2.2|env{mid=b7,sid=b,seq=7,ttl=P1D}|@chthonya>mac0sh|now|shared;task{id=t1,status=result,claim=repo_clean};χ=read_only;may=read_only;out=reported",
    ...
    task_result_blocked = not reports[6].executable and "task_result_without_evidence_ref" in reports[6].errors
    task_result_with_ref_ok = reports[7].executable
    no_permission_promotion = all("permission_promotion" not in r.errors for r in reports)
    gate = "pass" if roundtrip_ok and payload_blocked and validation_adjunct_blocked and validation_authz_variant_blocked and task_result_blocked and task_result_with_ref_ok and malformed_blocked and no_permission_promotion else "fail"
    common = {
        "gate": gate,
