...
The dialect discriminant is the first executable evidence slot and must be canonical. `ψ=CL2.v2.2` is the normal CL2.v2.2 surface; the existing ASCII compatibility spelling `psi=CL2.v2.2` remains parseable. Leading or trailing whitespace inside the discriminant value is ambiguous and invalid: `ψ= CL2.v2.2|...`, `ψ=CL2.v2.2 |...`, and `psi= CL2.v2.2|...` fail before canonical AST/policy evaluation with `malformed_dialect_discriminant`. Parser defaults are never allowed to manufacture or silently trim dialect evidence.
