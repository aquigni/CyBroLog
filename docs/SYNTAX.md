...
Frame slots after the route are also explicit in executable CL2 records. The `time` slot must exist and be non-empty after trimming, the `scope;field...` slot must exist, and the scope before the first `;` must be non-empty. The semicolon is required even when the semantic body is intentionally empty: `@mac0sh|2026-06-05T05:34:00Z|shared;   ` is valid, while `@mac0sh`, `@mac0sh|now`, `@mac0sh||shared;...`, `@mac0sh|now|;...`, and `@mac0sh|now|shared` fail before canonical AST/policy evaluation with `malformed_frame_slot`. Parser-renderer defaults such as `now` or `shared` are never allowed to manufacture missing frame evidence from parsed executable input.
