...
```text
ψ=CL2.v2.2|env{mid=c22-01,sid=cybrolog22,seq=1,corr=activate,ttl=PT2H}|@chthonya>mac0sh|now|shared;⟦REQ<peer_review>⟧;TOP=CyBroLog;ka=C;sam=M;kar=tests;cmp{id=cmpA,mode=full,target=cybrolog_surface,semantic_policy=lossless_ast,status=validated};val{id=valA,checks=[parse_roundtrip,ast_equivalence,exact_zone_recall],result=pass};χ=read_only+P0_preserved;may=read_only;π=PO{id=po22,owner=mac0sh,required=[critique,delta_cavetest],state=open};out=req
